...
from rag.agent import load_full_knowledge, ask_rag, ask_rag_async
...
load_dotenv()
ALLOWED_GUILDS = {1449773237595537601}
KNOWLEDGE = None
ERROR_MSG = "Error: variable {} is missing."
DISCORD_TOKEN = os.getenv("DISCORD_TOKEN") or sys.exit(ERROR_MSG.format("DISCORD_TOKEN"))
API_KEY = os.getenv("OPENAI_API_KEY") or sys.exit(ERROR_MSG.format("OPENAI_API_KEY"))
ID_MODEL = os.getenv("ID_MODEL") or sys.exit(ERROR_MSG.format("ID_MODEL"))
...
with open("cfg/prompt_2025-12-22.txt", encoding="utf-8") as f:
    PROMPT = f.read()
...
async def on_ready() -> None:
    ...
    for guild in bot.guilds:
        ...
        if guild.id in ALLOWED_GUILDS:
            ...
            count = await parse_guild(guild)
            print(f"Saved {count} messages")

        events_logger.info(f"Members in {guild.name}:\n" + "\n".join(members_info))

    global KNOWLEDGE
    KNOWLEDGE = await load_full_knowledge(ALLOWED_GUILDS)
    print("Knowledge loaded")
    results = KNOWLEDGE.search("hunting")
...
#########################################################################
#  None of these two work and I will maybe figure it out in the future. #
#  Or not.                                                              #
#                                                                       #
# @bot.before_invoke                                                    #
...
async def ping(ctx: commands.Context) -> None:
    ...
    The command is meant to be restricted for the intentional servers, so it shall work
    only during tests.
    
    Args:
        ctx (commands.Context): The context of the command invocation, including message,
        author, and channel.
    
    Returns:
        None (NoneType): This function sends a test message to the channel and does not
        return a value.
    """
    reply = "pong"
...
async def hello(ctx: commands.Context) -> None:
    ...
    Returns:
        None (NoneType): This function sends a greeting message to the channel and does
        not return a value.
    """
    reply = "Hello!"
    await ctx.send(reply)
...
async def on_command_error(ctx: commands.Context, error: commands.CommandError):
    ...
    Returns:
        None (NoneType): This function sends a warning message when user misused the
        command and does not return a value.
    """
    errors_logger.error(
...
async def on_message(message: discord.Message) -> None:
    """
    Handles any of the input messages from users that's not a preset command that can be
    called with prefix.
    Currently warns user about not communicating in case of DM attempt.
    Main purpose of that is to allow the bot to send RAG-requests if the intentional
    ...
    if bot.user.mentioned_in(message):
        if message.guild and message.guild.id == 1449773237595537601:
            if KNOWLEDGE is None:
                ...
                reply = await ask_rag_async(message.content, ID_MODEL, PROMPT, KNOWLEDGE)
        else:
